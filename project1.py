...
class MRWordFrequencyCount(MRJob):
    ...
    def tokenizeReview(self, review):
        ...
        self.categories_tokens.setdefault(review['category'], {})
        self.categories_tokens.setdefault('category_count', {})
        self.categories_tokens['category_count'].setdefault(category, 0)
        self.categories_tokens['category_count'][category] = self.categories_tokens['category_count'][category] + 1
        ...
        for token in tokens:
            self.categories_tokens[category].setdefault(token, 1)
            self.categories_tokens[category][token] = self.categories_tokens[category][token] + 1
    ...
    def mapper_count_words(self, _, line):
        for review in line.splitlines():
            review = json.loads(review)
            self.tokenizeReview(review)


    def mapper_final_get_words(self):
        for category, words in self.categories_tokens.items():
            for word, apps in words.items():
                yield (category, word), apps

        for category, counts in self.categories_counts.items():
            yield ('categories_counts', category), counts
